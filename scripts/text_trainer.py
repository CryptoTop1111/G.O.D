...
def get_training_log_summary(log_path: str) -> dict:
    """
    Parse training log to verify training actually ran and extract last loss/step for debugging.
    Returns dict with: ran (bool), last_loss (float|None), last_step (int|None), error_type (str|None).
    Tip: set logging_steps=1 in your train config for first runs so loss appears in the log quickly.
    """
    out = {"ran": False, "last_loss": None,
           "last_step": None, "error_type": None}
    if not os.path.exists(log_path):
        return out
    with open(log_path, "r") as f:
        text = f.read()
    out["error_type"] = OOM_ERROR if OOM_ERROR in text else (
        VLLM_OOM_ERROR if VLLM_OOM_ERROR in text else None)
    # Trainer logs like 'loss': 1.23 or "loss": 1.23 (allow scientific e.g. 1.2e-01)
    loss_matches = list(re.finditer(
        r"[\"']loss[\"']\s*:\s*([\d.eE+-]+)", text))
    # Trainer may log 'step' or 'global_step'
    step_matches = list(re.finditer(
        r"[\"'](?:global_)?step[\"']\s*:\s*(\d+)", text))
    if loss_matches:
        try:
            out["last_loss"] = float(loss_matches[-1].group(1))
            out["ran"] = True
        except ValueError:
            pass
    if step_matches:
        out["last_step"] = int(step_matches[-1].group(1))
        out["ran"] = True
    return out
...
def run_training(
    ...
    for i in range(retries):
        ...
        if i > 0:  # there was something wrong so we will reduce the batch_size
            ...
            print(
                f"[Retry {i}] Previous run reason: {error_type or 'unknown (check log)'}", flush=True)
        ...
        summary = get_training_log_summary(log_path)
        print(
            f"[Training verification] ran={summary['ran']} last_step={summary['last_step']} last_loss={summary['last_loss']} error_type={summary['error_type']}",
            flush=True,
        )
        if output_dir and os.path.exists(os.path.join(output_dir, "success.txt")):
            if not summary["ran"]:
                print(
                    "[Training verification] success.txt present but log shows no loss/step; consider inspecting log.",
                    flush=True,
                )
            return True
        if not summary["ran"] and summary["error_type"]:
            print(
                f"[Training verification] Training did not progress; last error: {summary['error_type']}", flush=True)
        time.sleep(5)
    return False
    ...
    for run in train_runs:
        if run["current_loss"] > lowest_loss:
            if os.path.exists(run["output_dir"]):
                print(
                    f"Deleting checkpoint {run['output_dir']} with loss {run['current_loss']}", flush=True)
...
def main():
    ...
    request_path = os.path.join(
        ds_folder, f"training_request_{args.task_id}.json")
    ...
    if is_openai_model(original_model_name):
        ...
        # install kernel
        run_cmd_with_log(
            "pip install kernels==0.9.0", os.path.join(
                ds_folder, f"install_kernel.log")
        )
        is_openai = True
    ...
    while True:
        ...
        if args.task_type == TaskType.GRPOTASK.value:
            ...
            if state["mode"] == "initial":
                ...
                if "lrs" not in state:  # first time of continue
                    ...
                    assert len(
                        state["lrs"]) == n_runs, f"Number of learning rates {state['lrs']} should be equal to number of runs {n_runs}"
                    state["runs"] = []
                ...
                if len(state["runs"]) < n_runs:
                    ...
                    c_train_info["train_request"]["checking_mode"] = "none"
                    index = np.argmin([run["current_loss"]
                                      for run in state["runs"]])
                    print(
                        f"BL;{index};{state['runs'][index]['current_loss']}; {state['lrs'][index]}", flush=True)
                    # replace_args_in_cmd(train_cmd, "learning_rate", str(state["lrs"][index]))
        ...
        if train_cmd:
            run_output_dir = output_dir + \
                f"_{count}" if not final_output_dir else final_output_dir
            train_cmd = replace_args_in_cmd(
                train_cmd, "output_dir", run_output_dir)
            ...
            train_cmd = replace_args_in_cmd(
                train_cmd, "request_path", current_request_path)
            ...
            state["train"]["start_time"] = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S")
            ...
            if not success:
                print(
                    f"Training failed for task {args.task_id} at count={count}", flush=True)
                break

        count += 1

    # Success = submission_dir exists and has at least 2 entries (checkpoint files/dirs + e.g. loss.txt)
    submission_ok = os.path.exists(submission_dir) and len(
        os.listdir(submission_dir)) >= 2
    if not submission_ok:
        print(
            f"Training failed for task {args.task_id} (submission_dir missing or <2 entries)", flush=True)
        log_path = os.path.join(ds_folder, f"train_{args.task_id}.log")
        if os.path.exists(log_path):
            s = get_training_log_summary(log_path)
            print(
                f"[Failed run summary] last_step={s['last_step']} last_loss={s['last_loss']} error_type={s['error_type']}", flush=True)
    else:
        print(
            f"Training successfully done for task {args.task_id}", flush=True)
        train_success = True
    ...
    if not train_success:
        ...
        run_cmd_with_log(
            add_noise_cmd, os.path.join(
                ds_folder, f"add_noise_{args.task_id}.log")
        )
